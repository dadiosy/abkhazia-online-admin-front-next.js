...
const DropZoneImage = ({ onChildData }) => {
    ...
    const axiosFunc = async (imgFile) => {
        ...
        setUploading(true);
        try {
            const res = await axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}/img/meta`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
            onChildData(`${process.env.NEXT_PUBLIC_API_BASE_URL}/img/meta/${res.data.data}`);
        } catch (error) {
            console.log(error);
